...
| [miniflux-cli](miniflux-cli/) | Manage a [Miniflux](https://miniflux.app/) RSS reader via a pure-TypeScript CLI (`skill/SKILL.md`). 24 subcommands covering reads (feeds, entries, categories, users, discovery, OPML export) and writes (mark read, bookmark, feed/category CRUD, refresh, OPML import); reads `MINIFLUX_URL` + `MINIFLUX_API_KEY` from the environment. |
