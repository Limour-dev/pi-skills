...
| [pubmed-search](pubmed-search/) | Search PubMed via NCBI E-utilities with a zero-dependency, pure-TypeScript client (`search`, `getByPmid`, `lookupMesh`). Ported from nature-skills' `pubmed.py`; runs directly on Node ≥ 23.6 or `npx tsx` — no Python, no MCP server needed. |
